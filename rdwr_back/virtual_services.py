...
class VirtualServer(lb_base):
	...
	def create_virtual_server(self,ip_addr,name="VirtualServer Create From API"):
		index = 1
		headers = {
	...
	def create_virtual_service(self,vport,rport,vindex,rindex,**kwargs):
		# Virtual Services are created with default values, 
		# we can also pass kwargs for custom requirements
		# ---- customization can be done for action,delayed binding, persistent mode
		headers = {
				'content-type': "application/json",
				'cache-control': "no-cache",
				}
